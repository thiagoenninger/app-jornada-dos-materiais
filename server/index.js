...
function sendApp(res, fileName) {
    ...
    if (!fs.existsSync(file)) {
        res.status(503).type('text/plain; charset=uft-8').send(`O front-end ainda não foi compilado.\n\n` + `Rode:  npm run build\n\n` + `(esperava encontrar dist/${fileName})`)
        return;
    }
    res.sendFile(file);
}
...
function handleHello(socket, message) {
    ...
    if (role === Role.SCREEN) {
        for (const other of wss.clients) {
            if (other !== socket && other.meta && other.meta.role === Role.SCREEN && other.meta.screen === screen) {
...
server.listen(PORT, HOST, () => {
    ...
    console.log('')
    console.log(`  Materiais habilitados: ${config.enabledMaterials.map((m) => m.id).join(', ') || '(nenhum'}`)
    console.log('')
})
